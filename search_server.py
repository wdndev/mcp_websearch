...
class WebSearchResult(BaseModel):
    """Search result model from search engines."""
    url: str
    title: Optional[str] = None
...
@mcp.tool(
    name="search_web_data_from_query", 
    description="Search web content using a search engine. Supports: ddgs, bing, baidu, google"
)
async def search_web_data_from_query(
    query: str,
    num_results: int = 5,
    search_type: Literal["ddgs", "bing", "baidu", "google"] = "ddgs",
)  -> List[WebSearchResult]:
    """ Search results for a given query using specified search engine.
    :param query: The search query
    :param num_results: The number of search results to retrieve
    :param search_type: The search engine to use
    :return: A list of WebSearchResult objects
    """
...
@mcp.tool(
    name="fetch_web_data_from_url", 
    description="Scrape web content from a specific URL"
)
async def fetch_web_data_from_url(
    url: str, 
    ctx: Context=None
) -> WebScrapeResult:
...
async def fetch_web_data_from_query(
    ...
    :return: A list of WebScrapeResult objectss
    """
    ...
    for search_result, content in zip(search_results, scraped_data):
        ...
        results.append(WebScrapeResult(
